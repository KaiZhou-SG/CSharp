<#
  ...
  Requirements:
    Update the source files under current folder($dirCurrent) to those from CVS($dirTarget)
  Specifications:
    ...
    (5) If there're files under $dir_target but not $dirCurrent, copy them from $dirTarget to $dirCurrent
  ...
  Design:
    ...
    (3) Based on $dirTargetItem and $dirCurrent, generate the $dirCurrentItem, which 
    (4) Following step (3), copy $dirTarget\subDir\foundFile.cs and paste to dirCurrent\subDir\foundFile.cs
